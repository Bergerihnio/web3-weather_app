...
def get_stats():
    ...
    c.execute("SELECT temperature FROM temperatures WHERE date= (?) AND time BETWEEN ? AND ?", (data, time_start, time_end))
    rows = c.fetchall()
...
def get_stats_sql(offset):
    conn = sqlite3.connect('statistics.db')
    c = conn.cursor()
    c.execute("SELECT median_temp, date FROM statistics ORDER BY DATE DESC LIMIT 1 OFFSET (?)", (offset,))
